...
machine is substantially slower; that was not measured.
...
| Postgres engine tests on a cold machine | `just ci-full` | ⚠ `postgres_engine.rs:100` prints `SKIP` and **passes with no assertion** when `~/.theseus` lacks the PG binaries (MEASURED by Bolt; didn't fire on a warm cache) |
...
| Embedded Postgres binaries hash-pinned | `KNOWN_GOOD_HASHES` and `verify_pinned_artifacts` in `engine.rs` | **Pinned for `aarch64-apple-darwin` and `x86_64-unknown-linux-gnu` only**; other platforms refuse to start |
...
- **First run downloads** Postgres 16.4.0 (theseus-rs GitHub releases) into `~/.theseus/postgresql/`
  ...
  ⚠ On an empty `~/.theseus` the pgvector pin check may run before the install and fail as
  `hash-pin mismatch` (INFERRED, not executed).
