...
[P-0008-admin-token-shape](P-0008-admin-token-shape.md) locked the admin token structure: opaque 32-byte random, BLAKE3-hashed in the `admin_tokens` table, workspace claim from server-side row lookup. That ADR deferred the role model and permission shape to this slot, noting: "The role model downstream can add scopes and permission shapes, but the workspace claim anchor is [P-0008's] decision."

This ADR locks what the `scopes` array in the `admin_tokens` table means — i.e., the role enum, the permission matrix per role across MCP verb categories and CLI control-plane operations, and how the V0 application-layer enforcement (per [P-0006-v0-tenant-enforcement](P-0006-v0-tenant-enforcement.md)) maps roles to `WorkspaceCtx` population.

The decision also depends on [P-0001-storage-layout](P-0001-storage-layout.md) because the policy-surface count under C1 is small and auditable (~10 artifact tables, one RLS policy per table). Under C2 (composite-with-typed-slots) the policy surface would have been per-aspect, producing a significantly richer authorization matrix.

The brief's Hard constraints: multi-tenancy structural with `workspace_id` NOT NULL on every artifact table; RLS column-shape ships at V0; policy enforcement V0.1+. [P-0006-v0-tenant-enforcement](P-0006-v0-tenant-enforcement.md) named the V0 enforcement mechanism (typed `WorkspaceCtx` parameter binding at host-fn boundary). This ADR names the role model the `WorkspaceCtx` encoding carries.

## Decision Drivers

- **V0 scope: application-layer enforcement only.** RLS policy enforcement deferred to V0.1+ per accepted risk `R-0001`. The role model must be implementable entirely at application layer (host-fn boundary enforcement, WHERE-clause mandatory discipline) without Postgres RLS policies active. The V0.1 migration path hardens the same role model at substrate layer — the role enum and permission matrix must be stable across that migration.
- **Policy-surface count under C1.** Under C1 single-document layout (~10 artifact tables), the RLS policy surface per role is manageable: one policy per (role, table) pair. The role enum must not explode this surface.
...
- **Threat coverage.** Multiple `{{P-RLSAdminToken}}` threat entries in the architecture overview address: workspace-lifecycle ops behind admin scope (`P-builtin-workspaces`/E); workspace-claim extraction defaulting to `default` workspace (`P-builtin-auth`/E); per-token attribution on writes (`EE-orchestrator-agent`/R); token-file inode pinning and modification fail-shut (`DS-admin-token`/T).
- **V0 is single-operator, single-workspace dogfood.** The role model must support V0 without over-engineering. A minimal role set (admin + read-observer) covers the V0 use cases; additional roles (e.g., per-plugin scope grants, per-team-member roles) are V0.1+ work.
- **Admin scope distinct from user scope.** Workspace lifecycle ops (create, delete) require a scope that ordinary verb-invoking tokens do not carry. The admin scope is a strict superset of user scope, distinguished by claim.
...
Enforcement at V0 is application-layer (host-fn boundary per [P-0006-v0-tenant-enforcement](P-0006-v0-tenant-enforcement.md)). The matrix below defines what each role authorizes. At V0.1+, these same categories map to Postgres RLS policies (one per artifact table per role).
...
These are not configuration — they are structural invariants enforced at the host-fn boundary.
...
At V0, the permission matrix above is enforced at the application layer only: host-fn boundary checks via `WorkspaceCtx.role` + WHERE-clause mandatory discipline on every read path. The RLS column shape (`workspace_id NOT NULL` on every artifact table) ships at V0.

At V0.1+, the same role model hardens at substrate layer:
...
The `mnemra.workspace_id` and `mnemra.role` session settings are set by the host at request boundary (via `SET LOCAL`) using the same `WorkspaceCtx` values. The application-layer and substrate-layer policies are structurally identical; V0.1 activation is additive — no schema migration required.

The trip-wire for V0.1 activation: accepted risk `R-0001` — "first deployment serving more than one workspace (production multi-tenant traffic), OR the column-shape's lint coverage drops below 100% on read paths, OR a third-party plugin is loaded at runtime."

### Consequences

**Good:**
- `P-builtin-auth`/E (Critical): workspace claim is DB-row-sourced, NOT NULL enforced; default-to-default-workspace bug cannot arise.
- `P-builtin-workspaces`/E (Critical): workspace lifecycle ops gate on `Role::Admin` at host-fn boundary; `Role::ReadObserver` tokens cannot create/delete workspaces.
- `EE-orchestrator-agent`/R (High): `token_id` in `WorkspaceCtx` enables per-token write attribution; audit trail names token, not just workspace.
...
- Binary role enum keeps the policy surface manageable: ~20 Postgres RLS policies at V0.1 (2 roles × ~10 artifact tables), each straightforward.

**Bad / Trade-offs:**
- Binary role enum (admin / read_observer) provides no per-plugin granularity. An `admin` token can invoke verbs across all `core: true` plugin families. Fine at V0 dogfood (single operator); will need extension at V0.1+ when multi-tenant onboarding requires per-plugin scoping.
- `read_observer` role has no write access at all, not even to its own session metadata. This may be too restrictive for some V0.1+ monitoring use cases; scope extension is an ADR amendment.
- Token rotation does not currently invalidate in-flight requests that cached the old token (at V0 with no in-process cache per [P-0008-admin-token-shape](P-0008-admin-token-shape.md), this is not a problem — each request does a fresh DB lookup; rotation is effective immediately on next request).

## Pros and Cons of the Options

### R1 — Binary admin/read-observer + scope strings (accepted)

- Pro: V0.1 RLS hardening is additive; same role model at application and substrate layer.
- Pro: `WorkspaceCtx.token_id` adds write attribution without extra complexity.
- Pro: Binary role keeps policy surface ~20 policies at V0.1.
...
- Con: RLS policy surface explodes: (roles × plugins × artifact types) instead of (roles × artifact tables).
...
- Con: Does not address `EE-specialist-agent`/S (Medium, 70): sub-agent identity separation requires at minimum a read-scoped token.
...
- [P-0003-plugin-manifest](P-0003-plugin-manifest.md) — verb-to-scope mapping: plugin manifest declares exposed MCP verbs; host routing checks role against verb before dispatching to plugin.
