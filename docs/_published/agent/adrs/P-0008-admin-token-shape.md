...
Mnemra-core requires a static admin token at V0 as a bootstrap path for first-run and solo deployments. The token is the credential for the admin CLI and for MCP clients presenting admin-scope requests. Every token carries a workspace claim that scopes storage by workspace.

The product brief's Hard constraints commit to the static admin token with a structural workspace claim, but do not specify the token's internal structure. Two structurally different shapes are available:
...
The Frame's M4 direction (Stage 2a) split `{{P-RLSAdminToken}}` to surface this decision at Frame altitude because the token structure gates downstream security architecture: loss of the token file, rotation semantics, and workspace-claim binding all differ between the two shapes. This ADR locks the Tier-A half. `{{P-RLSAdminToken}}` (Tier A, paused pending `{{P-StorageLayout}}`) owns the role model and permission shape downstream.
...
- `P-builtin-auth`/E (Critical) mitigation: workspace claim is DB-row-sourced; absence is structurally impossible (NOT NULL column); default-to-default-workspace bug cannot arise from a NULL row field.
...
**Bad / Trade-offs:**
- Every authenticated request incurs a DB lookup for the token hash. At V0 dogfood rates this is not a bottleneck; if it becomes one, Option C (in-process cache with invalidation) is the natural V0.1+ optimization.
- BLAKE3 is a fast non-cryptographic hash for the token hash. Using a password-hashing function (Argon2, bcrypt) would be more resistant to offline brute-force if the DB row is leaked. However, the token is 256-bit random — brute-force is computationally infeasible regardless of the hash speed. BLAKE3 is the correct choice for a high-entropy token; password-KDFs are for low-entropy user passwords.
...
- Con: V0 dogfood load does not warrant the optimization; premature complexity.
