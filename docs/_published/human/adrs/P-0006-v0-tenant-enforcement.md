...
In mnemra-core, the `workspace_id` scoping key is structural from V0. Every artifact table carries `workspace_id NOT NULL`, and the Row-Level Security (RLS) column-shape ships at `0.1.0`. The RLS **policy enforcement** is a separate matter, and it's deferred to V0.1+. The Postgres policy objects aren't activated at V0. Turning them on before the application layer is stable would create a substrate migration cliff at V0.1+.

The accepted-risk register captures `R-0001`: "Cross-tenant disclosure on `P-mcp-handler`/I, `P-builtin-projects`/I, `DS-pg-content`/I, `DF-host-fn-call`/I depends on the host-side WHERE-clause discipline rather than database-enforced policy." The risk-register trip-wire fires at the first deployment serving more than one workspace (production multi-tenant traffic), or if lint coverage on read paths drops below 100%, or if a third-party plugin loads.

The workspace principle `P-SecurityLayered` requires that each security layer is independently load-bearing. So the application layer at V0 isn't a placeholder that RLS "will fix later." It's the operating enforcement mechanism, and its failure modes have to be structurally contained.

Three enforcement postures are in scope. The [Frame](../glossary.md#frame)'s Tier-A description calls typed `WorkspaceCtx` parameter binding the conservative lead, with two open variants. This [ADR](../glossary.md#adr) (a project-scoped [P-* ADR](../glossary.md#p--adr)) locks the choice.
...
A middleware layer intercepts all Postgres queries issued by the host and injects `WHERE workspace_id = ?` based on the active session. Host-fn authors don't need to explicitly thread `workspace_id` through parameters; the rewriter handles it.
...
Option A gives the strongest structural guarantee: the host-fn ABI itself enforces context threading. It's impossible to author a host-fn implementation that can issue a database query without having received a `WorkspaceCtx`. The parameter is structurally required, not optionally documented. This directly satisfies `P-host-fns`/T (Critical, severity 90): "`workspace_id` is host-derived from the calling session, NEVER a plugin parameter on write paths."

Option B (query rewriter) provides a weaker guarantee. The rewriter can be misconfigured, disabled in a test context, or bypassed by a direct query path (for example, a migration script, a backup tool, or a health probe). At V0 single-workspace dogfood the risk is low. But the rewriter approach normalizes the premise that workspace enforcement is a database-layer concern, which conflicts with `P-SecurityLayered`'s requirement that the application layer is independently load-bearing. It's also harder to verify: "the rewriter runs on every query" is harder to assert at code review than "every host-fn signature requires `WorkspaceCtx`."

Option C (per-fn explicit parameter) is weaker than Option A because the `workspace_id` parameter on write paths is exactly the attack surface `P-host-fns`/T names. A plugin that supplies `workspace_id` directly on a write call shouldn't even compile against the host-fn ABI. Option C inverts this: it requires plugins to supply `workspace_id` and trusts the validation inside the host-fn to catch mismatches. That's an advisory guard, not a structural one.
...
The `R-0001` trip-wire fires at the first deployment serving more than one workspace with production traffic. When it fires:

1. RLS policy objects must be defined and activated in a migration before the multi-workspace deployment proceeds.
2. `WorkspaceCtx` threading remains in place after RLS activation. It's the application-layer redundancy that `P-SecurityLayered` requires even after the substrate layer is active.
...
- Uniform threading across host-fns and builtins, with no "internal" bypass path.
...
- Test harnesses must construct a valid `WorkspaceCtx` for every host-fn test. A test-only constructor (bypassing token validation) is required, and it must be gated to `#[cfg(test)]` to prevent use in production code.
...
- Pro: Compatible with future RLS activation. The application layer doesn't go away, it becomes redundant redundancy (good).
