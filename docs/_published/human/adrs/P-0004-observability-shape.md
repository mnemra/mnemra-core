...
`deprecated`. This decision was accepted once, and it no longer applies, because the context it locked against changed. 2026-06-09.

This Architecture Decision Record (ADR: a record of a significant technical or process decision, its context, the rejected alternatives, and the expected consequences) is kept intact as the historical record. Its body is **not edited**, per the [ADR README](README.md) immutability rule and the template's frontmatter reference: an accepted ADR isn't rewritten to reflect new thinking. It's `deprecated` rather than `superseded` because there's **no successor *ADR*** to point at. The observability question was re-altituded out of the project-ADR layer entirely. Two later changes falsified the world this lock was made against. First, [P-0010](P-0010-storage-substrate-engine.md) (a project-scoped ADR, the P stands for project) D8 demoted TimescaleDB off the V0 stack, the same `dispatch_metrics`/events surfaces this ADR froze as hypertables, which definitively falsifies this ADR's storage core (hypertables, retention policies, continuous aggregates). So it can't return to `accepted`. Second, the maintainer ratified (2026-06-09, the E1 disposition) that observability is a **theory trait plus chassis mechanism, not a per-project ADR**: the host emits its telemetry storage-independently and never owns the service's own observability store. Under the constraint-graph when-to-lock edge, the falsified freeze is re-derived against the new world, not honored.

The generation-side decisions this ADR got right (the per-verb metrics surface, the health-endpoint body gating, event-schema versioning, telemetry no-leak) live now in the **architecture overview observability baseline**, a theory-trait baseline rather than an ADR (see [Architecture overview › Observability](../architecture/overview.md)). The *generation mechanism* routes to the chassis when it lands. This ADR has no `superseded_by` pointer because its content's new home is a non-ADR baseline, not a replacement ADR. The decision content below is the prior, deprecated record.

## Context and Problem Statement

Mnemra-core ships observability from `0.1.0`, following the workspace-wide principle: instrument before first user-touch. The frame document (output of stage 2 of the work-shaping pipeline, where agents walk the constraint graph and produce a constraint summary with its rationale chain) commits to per-verb metrics, structured logs, and a health endpoint, all backed by TimescaleDB hypertables with retention policies. The concrete surface has to be locked before the `0.1.0` substrate build begins: which metrics dimensions, what retention windows, what the health-endpoint response body looks like, and what continuous-aggregate windows are defined.
...
The frame's threat model (via the security reviewer) adds a privacy dimension. Per-verb metrics must not capture artifact bodies, and audit events must not carry content fragments (`P-host-fns`/R, `DS-ts-events`/I, `DS-pg-logs`/I). The observability shape is a mitigation surface for those threats.
...
- **Telemetry no-leak.** An audit script over a dogfood-day's logs, traces, dispatch events, and session events must find zero artifact-body matches against the known-content corpus (quality-attribute scenario).
