...
The maintainer made both decisions recorded here. They sit in one record because they correct two halves of the same unratified proposal, the plugin substrate stack, that the build reversed on both halves without ever writing an [ADR (Architecture Decision Record, a record that captures a technical or process decision, its context, rejected alternatives, and consequences)](../glossary.md#adr).

- **Decision A (raw Wasmtime over Extism)** is recorded `accepted`-on-landing rather than `proposed`. It is already in the tree, merged in Tasks 20–22 (commits `8bb2326` "plugin runtime — manifest load, allowlist, wasmtime limits, epoch supervisor (Tasks 20-21)" and `d378118` "resource-limit trap handling — kill-and-replace recovery (Task 22)"). It is not waiting on a review gate. This ADR ratifies the choice retroactively, in the same spirit as [P-0011](P-0011-logging-facade.md)'s accept-on-landing for the `tracing` scaffold.
- **Decision B (official `rmcp` SDK over hand-rolled MCP)** is `accepted` as a forward decision. The maintainer locked the choice; Task 23 (the MCP-server work) implements it. The lock comes before the implementation, which is why this is `accepted` and not `proposed`. There is no open question for a gate to resolve, only the build to follow.

This ADR doesn't `supersede` or `override` any prior record. The proposal it corrects, an earlier unratified internal research proposal ("ADR-001: Plugin runtime — Extism on Wasmtime, with rmcp 1.5"), carries **Status: Proposed** and was never ratified into this project's accepted ADR series. P-0001 through P-0011 contains no plugin-runtime or MCP-SDK ADR. A never-accepted research proposal can't be superseded, so the relationship is narrated in Context, and `supersedes`/`overrides` stay `null`.
...
Neither decision below re-opens a settled question. The maintainer settled both. They are recorded here under [P-PreserveDecisionSpace (keep the rejected options visible, since every ADR lists the alternatives the decision chose among)](../glossary.md#p-preservedecisionspace), the surfaced alternatives the lock chose among, not re-derived.
...
- **R-0010-a mandates conformance to an external wire contract.** [R-0010-a](../../specs/2026-05-24-mnemra-core-v0-substrate.md) is "The system SHALL run a single MCP server using the MCP specification 2025-06-18 with stdio transport at V0." (An [R-code is a stable identifier for a numbered requirement in a specification document](../glossary.md#r-codes); its full text is the line quoted here.) Conformance to a versioned external protocol is exactly what an official, conformance-tested SDK provides and what a hand-roll silently risks getting subtly wrong. (Decision B.)
...
- Decision A — shipped (paths confirmed to resolve in-repo, under `libs/mnemra-host/plugin/`):
  ...
  - Wasmtime dependency pin: `libs/mnemra-host/Cargo.toml` (`wasmtime = "=45.0.2"`, features `cranelift` + `component-model`; no `extism` dependency present).
