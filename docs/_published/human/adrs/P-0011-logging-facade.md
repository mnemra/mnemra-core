...
The maintainer made this decision, and it was implemented and merged in commit `b2830da` ("build(mnemra): tracing logging scaffold (facade-from-binary)") in this same increment. This ADR records the merged decision and its crate-topology contract. It's `accepted` on landing rather than `proposed` because the choice already sits in the tree; it isn't waiting on a review gate. It doesn't override a [`DEFAULTS.md`](DEFAULTS.md) baseline entry (DEFAULTS.md is the project's frozen architectural baseline, where each entry is a one-time snapshot of an upstream workspace decision).

This is a **dependency-selection + crate-topology** decision: which crate provides the logging plumbing, and where in the crate graph the subscriber lives. It is **not** an observability-*shape* decision. It locks no metric, no event schema, no telemetry sink, and no emission semantic. The observability *shape* (emission semantics, per-verb metrics, sink/storage) was re-altituded out of the project-ADR layer to the [observability baseline](../architecture/overview.md#observability) by the 2026-06-09 E1 disposition, and is deferred here to Task 25 / R-0004 (R-0004 is a numbered requirement code; its full text lives in the requirements document that defines it). See [Scope — foundation only, impl-crate-only](#scope--foundation-only-impl-crate-only) below. The home rationale, why this is an ADR while observability shape is not, is recorded under [More Information](#more-information).

## Context and Problem Statement

mnemra-core ships observability from `0.1.0`, per the workspace-wide instrument-first principle ([`P-InstrumentBefore`](../glossary.md#p-instrumentbefore): every production surface ships instrumented before launch, with metrics, structured logs, and traces in place at first user-touch rather than added after the first incident; Observability value). Before any emission semantics can be designed, the foundation has to exist. That means a logging plumbing crate, a single place where the global subscriber is installed, and a discipline for *where* in the crate graph that subscriber lives, so that library crates don't each reach for their own logging implementation.

Two structural questions had to be settled to lay that foundation:

1. **Which crate provides the logging facade?** The library code was emitting diagnostics via ad-hoc `eprintln!` to stderr. That output is unstructured, unfilterable, and unroutable to a real sink.
...
- **Lock the contract, vary the implementation.** The seam between "library code emits a diagnostic" and "a concrete subscriber formats and routes it" is intrinsic to the logging layer's identity, and should be locked even though the routing details are deferred ([`P-LockContract`](../glossary.md#p-lockcontract): lock the contract that other code depends on so implementations stay swappable; lock what is intrinsic, even when not fully exposed until later). The `tracing` macros are that contract; the subscriber is the implementation behind it.
- **Library crates carry no implementation knowledge.** A library that installs or configures a subscriber can't be embedded cleanly. You get double-init, conflicting filters, format coupling. Libraries must depend on the facade macros only, never on a subscriber implementation ([`P-PerRepoFirst`](../glossary.md#p-perrepofirst) decoupling, where shared abstractions emerge from observed reuse rather than speculation; this is also the standard `tracing` ecosystem idiom).
- **Ecosystem-standard, license-clean plumbing.** The facade should be the de-facto Rust standard for structured, leveled, span-aware logging. That way the emission layer (OTel export, JSON formatting) composes from existing layers rather than bespoke code, and the dependency clears the license gate.
- **Smallest mature foundation now; emission semantics deferred on a named tripwire.** Lay only the plumbing and the topology at V0. Defer the emission layer to its own work item rather than over-specifying it here ([`P-Defer`](../glossary.md#p-defer) / DF1: defer mechanism choice until evidence forces it, where DF1 is the deferred-decision entry for this ADR; Simplicity).

## Considered Options

The maintainer settled the plumbing-crate choice and the topology. They're recorded here per [`P-PreserveDecisionSpace`](../glossary.md#p-preservedecisionspace) (every ADR keeps its rejected alternatives so future readers can see what was on the table), not re-derived.
...
- **The binary crate `cmd/mnemra` owns the global subscriber.** As the first action in `main`, it installs a `tracing-subscriber` registry with a JSON formatting layer writing to **stdout** and an `EnvFilter`. The filter reads `MNEMRA_LOG`, then `RUST_LOG`, then falls back to `info`. The install uses `try_init()`, so a repeated call (for example, tests sharing a process) is a no-op rather than a panic. (`cmd/mnemra/logging.rs`.)
...
**Explicitly OUT OF SCOPE here (deferred, see the named tripwire below):** the full emission layer. That covers emission semantics, per-verb metrics, event schemas, OTel / OTLP export wiring, telemetry redaction, and host-function-body logging. None of those is specified by this ADR. The observability *shape* those concern lives in the [observability baseline](../architecture/overview.md#observability) (a theory-trait baseline, not an ADR), and the V0 work that builds the emission layer on this foundation is its own work item.
...
**Bad / Trade-offs:**
- A library crate that genuinely needs to *configure* logging (rather than emit) can't, by design. The topology forbids library-level subscriber init. Accepted: that capability belongs to the binary, and no V0 library needs it.
- The emission semantics aren't locked here, so this ADR alone doesn't satisfy any observability quality-attribute scenario. It's the prerequisite foundation; the scenarios are met only once Task 25 / R-0004 builds the emission layer on it. This is the intended altitude split, recorded so the gap is named rather than silent.
...
**Home rationale — why this is a project ADR (and observability *shape* is not).** The 2026-06-09 E1 disposition ([P-0010](P-0010-storage-substrate-engine.md) D8; [ADR README](README.md) "Current ADRs") re-altituded observability **shape** — emission semantics, the metric/event/health generation decisions, and the storage/sink question — out of the project-ADR layer to the [observability baseline](../architecture/overview.md#observability), and `deprecated` the observability-shape ADR [P-0004](P-0004-observability-shape.md) (no successor ADR). That disposition's scope is the observability *shape*. This decision is categorically different. It's a **dependency-selection + crate-topology** decision (which crate provides the plumbing; where the subscriber lives), structurally the same kind of decision as [P-0010](P-0010-storage-substrate-engine.md)'s concrete engine choice behind a swappable trait (D5: "the storage surface is an engine-agnostic trait; the implementation varies behind it"). The ADR-vs-design-note criterion ([ADR README](README.md); agent-first: a reversal that forces downstream agent rework or external-consumer reference) is met independently: reversing the facade or the topology forces every library-crate macro site and the binary's subscriber install to change. So the facade decision belongs in the ADR layer; the observability *shape* it is the plumbing for does not.
