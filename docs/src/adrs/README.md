...
Nine Tier-A ADRs authored in WS-E-2 Stage 3, plus the storage substrate/engine ADR (P-0010) folded in 2026-06-08 from a post-spec-lock storage-engine evaluation. The 2026-06-09 observability re-derivation (P-0010 D8's E1 disposition — generation⊥storage separation) was **re-altituded out of the project-ADR layer**: observability is a theory trait + chassis mechanism, not a per-project ADR, so its decisions live in the [observability baseline](../architecture/overview.md#observability) and the original observability ADR P-0004 is `deprecated` (no successor ADR). All Tier-A placeholder slots resolved (see [`placeholder-resolution.md`](placeholder-resolution.md) for full status).
