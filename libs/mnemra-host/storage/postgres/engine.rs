...
impl EmbeddedEngine {
    ...
    pub async fn start() -> Result<Self, Box<dyn Error + Send + Sync>> {
        ...
        // KNOWN DEFECT (#3565), not the design: `.temporary(true)` below gives every
        // start a fresh data dir that is deleted on drop, and this constructor serves
        // the PRODUCTION host and `mnemra init` as well as tests, so stored data does
        // not survive a restart. Host data must persist on the local filesystem;
        // temporary engines are correct only in test fixtures. Do not copy these
        // settings as intended behaviour.
        //
        // Build settings: ephemeral port, temporary data dir (see defect above).
        ...
        // (Also part of the #3565 defect: a persistent data dir needs persistent
        // credentials, so this per-start derivation is not the intended design.)
